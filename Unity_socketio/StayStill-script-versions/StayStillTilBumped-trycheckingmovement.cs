...
public class StayStillTilBumped : MonoBehaviour {
	...
	void Start () {
		...
		// rb.maxDepenetrationVelocity = 8.0f;  
		// don't know what appropriate range is, tried 0.1 - 8.0, but doesn't seem to have any useful effect
	}
	...
	void FixedUpdate() {
		// try checking for acceleration below a certain amount, and turn physics off
//		if (!rb.isKinematic) {
//			if(rb.velocity.sqrMagnitude < .01 && rb.angularVelocity.sqrMagnitude < .01) { //(rb.IsSleeping()) {
//				Debug.Log (gameObject.name + " moving slow, is sleeping = "+rb.IsSleeping()+" time now is: "+Time.time+" and startTime was "+startTime);
//				rb.isKinematic = true;
//			}
//		}
	}
	...
	IEnumerator enoughAlready() {
		...
		if ((Time.time - startTime) < stopPhysicsAfter) {  // if i haven't hit timeout and...
			if (((Time.time - startTime) < 1.0) ||  // if i've just been hit or....
		    (rb.velocity.sqrMagnitude > .01 || rb.angularVelocity.sqrMagnitude > .01))  // if i'm still moving
		    { // keep going and set timer again
