...
//		Debug.Log ("StayStillTilBumped:enoughAlready() "+gameObject.name + " enoughAlready time is: "+Time.time+" and startTime was "+startTime+ " and sleeping = "+rb.IsSleeping());
		...
		if ((Time.time - startTime) < stopPhysicsAfter) {   // if haven't exceeded timeout
			if (((Time.time - startTime) < 1.0) ||  // if i've just been hit or....
				...
				// could add test to see if it is on the ground and not can't fall further?  not sure if that would be helpful
